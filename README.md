...
![Node.js](https://img.shields.io/badge/Runtime-Node.js_18+-339933?style=for-the-badge&logo=node.js&logoColor=white)
...
> *Note on this revision:* Per the agile team's decision, the backend has been migrated from **Python / FastAPI** to **Express.js on Node.js (TypeScript)**. All Python-specific tooling (Pydantic, SQLAlchemy/SQLModel, Alembic, Celery, uvicorn) has been swapped for their Node-native equivalents (Zod, Prisma, Prisma Migrate, BullMQ). Everything else in the brief — data model, API surface, business rules, compliance posture — is architecture-agnostic and unchanged.
...
| API Gateway | AuthN/AuthZ (JWT via `jsonwebtoken` + OAuth2 via `Passport.js`), RBAC enforcement (Express middleware), request validation (`Zod` schemas), rate limiting (`express-rate-limit`) |
| Agentic AI | Classification, document generation, routing, multilingual chat, scheme RAG |
| Data | Transactional data (Postgres via Prisma), ephemeral locks/cache (Redis), embeddings (vector store), files (object storage) |
| External | Payments, notifications, translation — all pluggable, none hard-coded to a single vendor |

> **Note:** classification/document-generation can take a few seconds. Return the Problem ID as soon as the record is created, and run the AI steps as a **BullMQ job** (Redis-backed queue) that updates status asynchronously — don't block the citizen's confirmation screen on an LLM round-trip.
...
| *Backend* | **Express.js**, **Node.js 18+ (LTS)**, **TypeScript**, **Zod** (schema validation) |
| *ORM / Migrations* | **Prisma** (Prisma Client + Prisma Migrate) targeting PostgreSQL |
| *Async task queue* | **BullMQ + Redis** (AI pipeline jobs, notifications, nightly HPI computation) |
| *Agent orchestration* | **LangChain.js** / **LangGraph.js** (`@langchain/langgraph`) |
...
| *RAG / vector store* | pgvector (Postgres extension, queried via Prisma raw SQL / `node-postgres`) or a managed vector DB (Pinecone/Weaviate) |
| *Primary database* | PostgreSQL 15+ |
| *Cache / slot locking* | Redis |
| *Object storage* | AWS S3 or Cloudinary (grievance documents, selfies) |
| *Payments* | Razorpay (UPI, cards, net banking) |
| *Auth* | JWT (`jsonwebtoken`) + OAuth2 (`Passport.js`), optional Aadhaar-based eKYC for identity verification |
| *API documentation* | `swagger-jsdoc` + `swagger-ui-express`, served at `/docs` |
| *DevOps* | Docker, Docker Compose, GitHub Actions, Nginx |
| *Monitoring* | Prometheus + Grafana (via `prom-client`), Sentry |
...
├── backend/                        # Express.js (Node.js + TypeScript) app
│   ├── prisma/
│   │   └── schema.prisma           # Prisma data model + migrations source
│   └── src/
│       ├── routes/v1/              # routers: problems, auth, appointments, tokens, hpi, chatbot
...
│       ├── jobs/                   # BullMQ queues & workers (AI pipeline, notifications, nightly HPI)
│       ├── config/                 # env config, logging, third-party clients
...
```
*(Modeled in `backend/prisma/schema.prisma` as Prisma models; Prisma Migrate generates and applies the underlying SQL migrations.)*
...
Full interactive documentation is generated from JSDoc-style route annotations via `swagger-jsdoc` and served at `/docs` using `swagger-ui-express`.
...
- JWT-based session auth (`jsonwebtoken`) with an OAuth2 password flow (`Passport.js`) and refresh-token rotation.
- Role-Based Access Control (RBAC) enforced at the API layer on every endpoint (Express middleware), not just hidden in the UI.
...
### Prerequisites
- Node.js 18+ (LTS) — used by frontend, mobile, **and** backend
- PostgreSQL 15+
- Redis 7+
...
npm run dev            # ts-node-dev / nodemon, defaults to http://localhost:8000
```
...
Visit `http://localhost:5173` for the app and `http://localhost:8000/docs` for the Swagger UI.
...
## 🤝 Contributing
Contributions are welcome. Please open an issue to discuss significant changes before submitting a pull request, and follow the existing code style — **ESLint/Prettier across the entire stack** (frontend, mobile, and backend are all TypeScript).
